...
This project is not meant to make AI replace learning. It uses AI as a retrieval coach, generative sense-making coach, metacognitive feedback partner, spaced-review scheduler, and project-transfer assistant.
...
6. **Spaced review needs a schedule, not a vague intention.**
   A practical course system should record the next retrieval date for each active concept, then remind the learner only when review is due. The schedule should be flexible: first retrieval after 1-2 days, second retrieval after 5-7 days, and longer review after 2-3 weeks only for shaky or high-value concepts.

7. **AI support should include withdrawal checks.**
...
8. **High-risk claims need evidence and uncertainty.**
...
| --- | --- | --- |
| Yan et al. (2024) | GenAI can support personalization, feedback, and assessment, but it can undermine agency if used as a crutch. | Guardrail: AI should preserve ownership. Templates ask for the learner's own version, what they owned, and how they validated the result. |
| Kestin et al. (2025) | Research-based AI tutoring can outperform in-class active learning in an authentic RCT when the tutor is deliberately designed. | `AI Socratic Feedback`: diagnose prior understanding, ask targeted questions, give minimal correction, and prompt retry. |
| Bastani et al. (2024) | GPT-4 improved assisted practice performance in a field experiment, but standard GPT-style support hurt later unaided performance; learning safeguards mitigated this risk. | `No-AI transfer check` asks for one tiny unaided task after feedback so the system tests ownership, not just assisted fluency. |
...
| Makransky et al. (2025) | GenAI should support generative sense-making, not just passive interaction. | After-class and project workflows require explanation, visualization, and application rather than passive AI summaries. |
| Guan et al. (2025) | Educational chatbots can scaffold self-regulated learning processes such as planning, monitoring, reflection, and adjustment. | `course-progress.md`, `real-use-log.md`, `quality-checklist.md`, and review reminders turn the system into a self-regulated learning loop. |
| Xia et al. (2024) | GenAI changes assessment by increasing the need for responsible learning, self-regulation, and integrity. | `After-assignment Reverse Card` focuses on concepts, mistakes, validation, ownership, and interview-safe explanation instead of only final answers. |
| Endres et al. (2024) | Constructive retrieval, especially self-generated elaboration, supports comprehension and metacognitive monitoring. | `Post-class Retrieval Card` and `复习好了` status updates ask for recall, self-explanation, confusion, and shaky/improving/solid status. |
...
| Roediger & Karpicke (2006) | Taking tests after studying improves long-term retention more than restudying, especially after a delay. | `Review Reminder` outputs active-retrieval prompts instead of telling the learner to reread lecture notes. |
| Cepeda et al. (2006) | Distributed practice improves retention, but the best gap depends on the target retention interval; there is no universal day count. | `Review Schedule` uses practical windows and adjusts by concept status rather than assigning every concept the same fixed interval. |
| Karpicke & Bauernschmidt (2011) | Repeated spaced retrieval improves long-term retention; overall spacing and repeated retrieval matter more than one exact expanding schedule. | `course-progress.md` stores first retrieval, second retrieval, and long-term review stages. |
| Butowska-Buczynska et al. (2024) | Spaced retrieval is stronger when retrieval varies across practice sessions. | `Spaced Review Queue` retrieves each concept through different contexts: formula, code, business, project, and interview. |
...
## v1.5 Spaced Review Basis

`course-progress.md`, `Review Schedule`, and `Course Review Reminder` extend the existing retrieval-first workflow into an executable spaced-review loop:

- Same day: finish with wrap-up, ownership, shaky points, and review scheduling.
- 1-2 days later: first active retrieval.
- 5-7 days later: second active retrieval.
- 2-3 weeks later: long-term review only for shaky or high-value concepts.
- Weekly review: triage due, overdue, shaky, and high-value concepts; do not reread all slides by default.

This is deliberately a flexible window-based schedule, not a rigid calendar law. Concept status, priority, deadlines, and learner energy can change the exact date.

## APA 7 References

Butowska-Buczynska, E., Klis, P., Zawadzka, K., & Hanczakowski, M. (2024). The role of variable retrieval in effective learning. *Proceedings of the National Academy of Sciences, 121*(44), Article e2413511121. https://doi.org/10.1073/pnas.2413511121
...
Guan, R., Rakovic, M., Chen, G., & Gasevic, D. (2025). How educational chatbots support self-regulated learning? A systematic review of the literature. *Education and Information Technologies, 30*, 4493-4518. https://doi.org/10.1007/s10639-024-12881-y
...
Yan, L., Greiff, S., Teuber, Z., & Gasevic, D. (2024). Promises and challenges of generative artificial intelligence for human learning. *Nature Human Behaviour, 8*, 1839-1850. https://doi.org/10.1038/s41562-024-02004-5
